...
char register_ext2_partition(const char* disk, uint32_t lba) {
    ...
    if (read_disk(disk, superblock_buffer, lba+2, 2)) {
    ...
    struct ext2_block_group_descriptor * block_group_descriptor = malloc(group_descriptor_size*block_groups_first);

    if (read_disk(disk, (uint8_t*)block_group_descriptor, lba+4, 2)) return 0;
